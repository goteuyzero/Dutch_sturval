...
let characterDraft = { roomCode: null, gameNumber: null, targetId: null, value: null };
...
function render() {
  ...
  const characterSelection = activeElementId === 'characterInput'
    ? {
  ...
  restoreCharacterInputFocus(characterSelection);
...
function getCharacterDraftValue(targetId, serverValue) {
  const sameDraft = characterDraft.roomCode === state.room_code
    && characterDraft.gameNumber === state.game_number
    && characterDraft.targetId === targetId;

  return sameDraft && characterDraft.value !== null ? characterDraft.value : serverValue;
}

function saveCharacterDraft(targetId, value) {
  characterDraft = {
...
function restoreCharacterInputFocus(selection) {
  if (!selection) return;
...
function renderAssigningMain() {
  ...
  const serverCharacter = alreadySet && targetPlayer.character_visible ? targetPlayer.character : '';
  const characterValue = getCharacterDraftValue(me.target_id, serverCharacter);
    ...
    <main class="main-panel">
      <div class="center-card wide">
        ...
        <form id="characterForm" class="character-form">
          <input id="characterInput" class="input character-input" maxlength="80" value="${escapeHtml(characterValue)}" />
...
function attachCharacterFormHandler() {
  ...
  if (!form || !input || !state?.me?.target_id) return;

  input.addEventListener('input', () => {
    saveCharacterDraft(state.me.target_id, input.value);
  });
  ...
  form.addEventListener('submit', async (event) => {
    ...
    saveCharacterDraft(state.me.target_id, character);
